...
// .wrangler/tmp/bundle-NcmQBH/checked-fetch.js
...
// .wrangler/tmp/pages-pLTAuT/functionsWorker-0.27366337708512334.mjs
...
async function onRequest(context) {
  ...
  try {
    ...
    if (!message) {
      return new Response(
        JSON.stringify({ error: lang2 === "en" ? "Missing 'message' field" : "Das 'message'-Feld fehlt" }),
        {
          status: 400,
          headers: {
            "Content-Type": "application/json",
    ...
    let systemPrompt = lang2 === "en" ? "You are a helpful AI assistant. Respond politely and informatively in English." : "Du bist ein hilfsreicher AI-Assistent. Antworte h\xF6flich und informativ auf Deutsch.";
    if (files.length > 0) {
      systemPrompt += lang2 === "en" ? `
...
- Best\xE4tige explizit, dass du die Dateien gelesen hast`;
    ...
    if (!apiResponse.ok) {
      ...
      if (errorText.includes("context_length_exceeded")) {
        const errorMsg = lang2 === "en" ? "Sorry, the uploaded files are too large. Please use smaller files or split them." : "Entschuldigung, die hochgeladenen Dateien sind zu gro\xDF. Bitte verwende kleinere Dateien oder teile sie auf.";
    ...
    const botAnswer = data.choices?.[0]?.message?.content || (lang2 === "en" ? "Sorry, I couldn't generate a response." : "Entschuldigung, ich konnte keine Antwort generieren.");
    try {
    ...
    const fallbackMsg = {
      ...
      choices: [{
        message: {
          content: lang === "en" ? "Sorry, there was a technical error. Please try again." : "Entschuldigung, es gab einen technischen Fehler. Bitte versuche es erneut."
        }
      }]
    };
...
- Best\xE4tige explizit, dass du die Dateien gelesen hast`;
    ...
    console.log("Response mentions files:", botAnswer.toLowerCase().includes("datei"));
    try {
      await saveToAirtable3(env, message, botAnswer, files, parsedBody.fileAttachments);
      console.log("Successfully saved to Airtable with bot answer");
    } catch (airtableError) {
      console.error("Airtable save failed:", airtableError);
    }
...
async function saveToAirtable3(env, originalMessage, botAnswer, files, fileAttachments = []) {
  const AIRTABLE_API_KEY = env.AIRTABLE_API_KEY;
  const AIRTABLE_BASE_ID = env.AIRTABLE_BASE_ID;
  const AIRTABLE_TABLE_NAME = env.AIRTABLE_TABLE_NAME || "Prompts";
  if (!AIRTABLE_API_KEY || !AIRTABLE_BASE_ID) {
...
__name(saveToAirtable3, "saveToAirtable3");
__name2(saveToAirtable3, "saveToAirtable");
var routes = [
  {
    routePath: "/ai",
    mountPath: "/",
...
// .wrangler/tmp/bundle-NcmQBH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-NcmQBH/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.27366337708512334.js.map
