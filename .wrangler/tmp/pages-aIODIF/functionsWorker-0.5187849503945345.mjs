...
// ../.wrangler/tmp/bundle-4hvWye/checked-fetch.js
...
async function onRequestPost(context) {
  ...
  try {
    ...
    const {
      ...
      files = [],
      prompt
      // This is the `gesamtPrompt` from your frontend Form
    } = body;
    if (!message) {
...
WICHTIG: Der Benutzer hat ${files.length} Datei(en) hochgeladen. Diese Dateien sind im Nachrichteninhalt unter "[Datei: ...]" zu finden. Lies und analysiere den Inhalt dieser Dateien sorgf\xE4ltig und beziehe dich in deiner Antwort darauf.`;
    }
    const chatMessages = [
      { role: "system", content: systemPrompt },
      ...messages,
      // Use the complete history array as sent by the frontend
    ...
    if (!apiResponse.ok) {
      ...
      if (errorText.includes("context_length_exceeded")) {
        ...
        return new Response(JSON.stringify(errorResponse), {
          status: 200,
          // Send 200 so the frontend can display the message
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
      }
...
// ../.wrangler/tmp/pages-aIODIF/functionsRoutes-0.016368990654578086.mjs
var routes = [
  {
    routePath: "/ai",
    mountPath: "/",
...
// ../.wrangler/tmp/bundle-4hvWye/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-4hvWye/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5187849503945345.mjs.map
